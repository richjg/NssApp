...
namespace NssApp.ViewModels
{
    public static class DataPointExtensions
    {
        ...
        public static List<HomeViewModel.DataSourceItem> AddMonthlyDataPointsAndOrder(this List<HomeViewModel.RestDataPoint> dataPoints, int months)
        {
            var normalisedDataPoints = new List<HomeViewModel.DataSourceItem>();
            var maxDate = dataPoints.Any() ? dataPoints.Max(dp => dp.DateTime) : DateTime.UtcNow;
            ...
            for (var i = -months + 1; i <= 0; i++)
            {
                ...
                normalisedDataPoints.Add(new HomeViewModel.DataSourceItem
                {
                    Label = calculationMonth.ToString("MMM"),
                    Value = dataForMonth?.Value.GetValueInScale(scaleUnit) ?? 0
                });
